...
Now you can rename `src/index.css` to `src/App.scss` and run `npm run watch-css`. The watcher will find every Sass file in `src` subdirectories, and create a corresponding CSS file next to it, in our case overwriting `src/index.css`. Since `src/index.js` still imports `src/index.css`, the styles become a part of your application. You can now edit `src/App.scss`, and `src/index.css` will be regenerated.
